(function() {
  ...
  function updateTestExec(testExec) {
    ...
    if ('total' in testExec.counters) {
      ...
      var countersString = '';
      // passed
      for (var i = 0; i < counters.passed; i++) { countersString += '+'; }
      // failed
      for (i = 0; i < counters.failed; i++) { countersString += '*'; }
      // skipped
      for (i = 0; i < counters.skipped; i++) { countersString += '-'; }
      // rest adding up to total
      for (i = countersString.length; i < counters.total; i++) { countersString += '.'; }
